...
public class Ex20SQLiteExLogin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ex20_sqlite_login_layout);

        //데이베이스 생성.
        helper = new MySQLiteOpenHelper(
                Ex20SQLiteExLogin.this, // 현재 화면의 context
                "member2.db", // 파일명
                null, // 커서 팩토리
                1); // 버전 번호
        ...
        public void onClick(View v)
        {
            switch(v.getId())
            {
                case R.id.btnLogin:
                    ...
                    if(loginCheck ==true)
                    {
                        Intent login = new Intent(Ex20SQLiteExLogin.this, Ex20SQLiteExMain.class);
                        startActivity(login);
                    }
                    else
                    {
                        Toast.makeText(Ex20SQLiteExLogin.this, "로그인 실패!(아이디/패스워드확인후다시하세요)",  Toast.LENGTH_SHORT).show();
                    }
                    break;
                case R.id.btnJoin:
                    Intent login = new Intent(Ex20SQLiteExLogin.this, Ex20SQLiteExJoin.class);
                    startActivity(login);
                    break;
            }
        }
